...
public class ConfigBase {
	...
	{	
		try {
			System.setProperty("webdriver.chrome.driver", "C:\\ChromeDriver80\\chromedriver.exe");	
			driver = new ChromeDriver();  //abre el navegador			
			driver.manage().window().maximize(); 
			driver.manage().deleteAllCookies();  
			driver.manage().timeouts().pageLoadTimeout(40, TimeUnit.SECONDS);// PageLoadTimeOut, espera X segundos  para que se muestra la página web (url).		
